...
for i, cell in enumerate(nb['cells']):
    ...
    # Cell 16 - Model class
    if 'class EnhancedAbAgModel' in src and 'ESM-2 for antigens' in src:
        print(f"Updating cell {i} - Model class (ESM-2 -> ProtT5)")
        cell['source'] = new_model_code.split('\n')
        cell['source'] = [line + '\n' for line in cell['source'][:-1]] + [cell['source'][-1]]

    # Cell 19 - Model build
    if 'use_esm2_3b=True' in src and 'EnhancedAbAgModel' in src:
        print(f"Updating cell {i} - Model build")
        cell['source'] = new_build_code.split('\n')
        cell['source'] = [line + '\n' for line in cell['source'][:-1]] + [cell['source'][-1]]

# Update the header markdown to reflect changes
for i, cell in enumerate(nb['cells']):
    src = ''.join(cell['source']) if isinstance(cell['source'], list) else cell['source']
    if '# Antibody-Antigen Binding Prediction - v2.7' in src:
...
---'''
        ...
        cell['source'] = [line + '\n' for line in cell['source'][:-1]] + [cell['source'][-1]]
        print(f"Updated cell {i} - Header (v2.7 -> v2.8)")
...
print()
print("="*60)
print("NOTEBOOK UPDATED: colab_training_v2.8_ProtT5.ipynb")
print("="*60)
print()
print("Changes made:")
print("  1. ESM-2 3B -> ProtT5-XL for antigens")
print("  2. Both encoders now 1024-dim (simpler architecture)")
print("  3. ProtT5 requires space-separated amino acids (handled)")
print()
print("Expected speedup:")
print("  - ESM-2 3B: ~2.5 hours/epoch")
print("  - ProtT5-XL: ~40-60 minutes/epoch")
print("  - Total training: 40 epochs in ~30-40 hours (vs 100+ hours)")
print()
print("Next steps:")
print("  1. Upload colab_training_v2.8_ProtT5.ipynb to Google Drive")
print("  2. Change Cell 11: CSV_FILENAME = 'agab_phase2_full_v2_balanced.csv'")
print("  3. Delete old checkpoints")
